...
export default function Documents() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (doc: DocumentRow) => {
      ...
      if (doc.file_url) {
        const storagePath = doc.file_url.startsWith("http") ? `${user!.id}/${doc.file_url.split("/").pop()}` : doc.file_url;
    ...
    <div className="animate-fade-in space-y-6">
      <PageHeader title="Documentos / IR" description="Cofre de documentos e módulo fiscal">
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button className="w-full sm:w-auto"><Plus className="mr-1 h-4 w-4" /> Novo documento</Button>
          </DialogTrigger>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={FileText} title="Sem documentos" description="Armazene recibos, comprovantes e documentos fiscais.">
          <Button onClick={() => setIsOpen(true)}><Plus className="mr-1 h-4 w-4" /> Adicionar</Button>
        </EmptyState>
      ) : (
        <Card>
          <CardContent className="divide-y divide-border p-0">
            {documents.map((doc: DocumentRow) => (
              <div key={doc.id} className="flex flex-col gap-3 p-4 transition-colors hover:bg-muted/30 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 flex-1 items-start gap-3">
                  <FileText className="h-5 w-5 shrink-0 text-muted-foreground" />
                  <div className="min-w-0">
                    <p className="line-clamp-2 break-all text-sm font-medium">{doc.file_name}</p>
                ...
                <div className="flex w-full items-center justify-between gap-2 sm:w-auto sm:justify-end">
                  ...
                  <div className="flex items-center gap-2">
                    {doc.file_url && (
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary" title="Visualizar documento (link seguro temporário)" onClick={() => handleViewDocument(doc)}>
                        <Download className="h-4 w-4" />
                      </Button>
                    )}
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => deleteMutation.mutate(doc)}>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input type="number" min="2020" max="2030" value={form.ano_fiscal} onChange={(e) => setForm((f) => ({ ...f, ano_fiscal: e.target.value }))} />
        </div>
      </div>
      <div className="space-y-2">
        <Label>Titular</Label>
        <Input placeholder="Nome do titular" value={form.holder} onChange={(e) => setForm((f) => ({ ...f, holder: e.target.value }))} />
      </div>
      <Button type="submit" className="w-full" disabled={submitting}>
        {submitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
        <Upload className="mr-1 h-4 w-4" /> Salvar documento
      </Button>
    </form>
  );
}
