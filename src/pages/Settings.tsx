        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input type="number" step="0.01" min="0" value={prefs.reserva_emergencia_valor || ""} onChange={(e) => setPrefs((p) => ({ ...p, reserva_emergencia_valor: Number(e.target.value) || 0 }))} />
            </div>
            <div className="space-y-2">
              <Label>Meta de cobertura (meses)</Label>
              <Input type="number" min="1" max="24" value={prefs.reserva_emergencia_meses_meta || ""} onChange={(e) => setPrefs((p) => ({ ...p, reserva_emergencia_meses_meta: Number(e.target.value) || 6 }))} />
            </div>
          </div>
          <div className="space-y-2">
            <Label>Renda mensal principal (R$)</Label>
            <Input type="number" step="0.01" min="0" value={prefs.renda_principal || ""} onChange={(e) => setPrefs((p) => ({ ...p, renda_principal: Number(e.target.value) || 0 }))} placeholder="Usada para cálculo de cobertura" />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input type="number" min="1" max="31" value={prefs.dia_fechamento || 1} onChange={(e) => setPrefs((p) => ({ ...p, dia_fechamento: Number(e.target.value) || 1 }))} />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {[
            ...
            <div key={key} className="flex items-start justify-between gap-4 rounded-lg border border-border/70 p-3">
