...
**Status (09/07/2026):** **Fase 0 implementada localmente** — modo Notificação/SMS, Share Target
PWA, origem `sms` e validação server-side de valor literal único. Publicação da Edge Function
`smart-capture-interpret` pendente de autenticação no painel Supabase. Próximo passo após uso
real: D.2 Bloco A (OFX/CSV); automação Android permanece atrás do gate de valor.
