...
Este projeto é acompanhado pelo Mentor de Projetos. Slug: `financeiroje`. O estado executivo vive em `.project-mentor/project.yaml` e o histórico em `.project-mentor/sessions/`. **Nunca edite esses arquivos à mão**: toda escrita passa pelo CLI `mentor`.
...
2. Rode `mentor project financeiroje --brief` e leia a última sessão em `.project-mentor/sessions/`.
...
4. Pergunte o objetivo só se não estiver claro. Depois rode `mentor start financeiroje --objective "..." --agent <claude-code|codex|antigravity|copilot>`.
...
- Ação nova: `mentor action add financeiroje --title "..."`. Concluir: `mentor action done financeiroje <act-id> --level implemented|tested`. Bloqueio: `mentor blocker add financeiroje --description "..."`.
...
2. Rode `mentor close financeiroje --from <rascunho.md>` e mostre o resultado da validação. Se falhar, mostre o erro e **não** finja sucesso.
